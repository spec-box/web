...
export const DependenciesFeaturesList: FC<DependenciesFeaturesListProps> = (props) => {
    ...
    const handleChangeOpen = useCallback(()=>{setIsOpen((prev)=> !prev)}, [])
    const arrow = (
    ...
    return (<div className={bem("Wrapper")}>
        <div onClick={handleChangeOpen} className={bem("Title")}>
            <span>Места использования </span>{' '}
            <span>{arrow}</span>
            </div>
